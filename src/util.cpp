...
const char * const BITCOIN_CONF_FILENAME = "clintoncash.conf";
const char * const BITCOIN_PID_FILENAME = "clintoncashd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "clintoncash" is a composite category enabling all clintoncash-related debug output
            if(ptrCategory->count(string("clintoncash"))) {
...
#else
    const char* pszModule = "clintoncash";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ClintonCashCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ClintonCashCore
    // Mac: ~/Library/Application Support/ClintonCashCore
    // Unix: ~/.clintoncash
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ClintonCashCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ClintonCashCore";
#else
    // Unix
    return pathRet / ".clintoncash";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty clintoncash.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override clintoncash.conf
